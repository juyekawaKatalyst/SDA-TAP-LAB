<script lang="ts">
  ...
  $: {
    ...
    $title = `<div class="flex flex-col">${CAT.OBJECT_NAME}<p style="background-color:${statusColor}" class="rounded h-1 w-full">&nbsp;</p></div>`;
  }
    ...
    if ((globalThis as any).viewer) {
      unsub = (globalThis as any).viewer.clock.onTick.addEventListener(
        (clock: Clock) => {
          ...
          $title = `<div class="flex flex-col">${CAT.OBJECT_NAME}
